...
- **Hub → worker is a pull model** (invariant 7). The hub issues material (cert-manager) and a hub-local `PushSecret`
  ...
  e.g. `worker-birth-kit/templates/openchoreo-agent-identity.yaml`. ESO then puts a `ces-<name>` finalizer on that
  namespace, so deleting the namespace needs the worker's ESO running. After removing a CES, check that the finalizer
  is gone (design doc addendum "data plane on workers as built").
