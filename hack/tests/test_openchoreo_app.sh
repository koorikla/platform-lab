...
dev=(-f $a/app.yaml -f $a/envs/dev/values.yaml --set env=dev)
...
# --- release (Kargo): ComponentRelease <app>-<env>-<tag>-<hash8>, frozen type spec + workload
...
assert_yq "$r" "$rel | .spec.componentType.spec.workloadType" deployment
...
# anything frozen changes the name: tag, env config, env, parameters
...
r4=$(render podinfo $c -f $a/app.yaml -f $a/envs/test/values.yaml --set env=test --set mode=release --set image.tag=6.15.0)
[[ "$(yq "$rel | .metadata.name" "$r4")" =~ ^podinfo-test-6-15-0-[0-9a-f]{8}$ ]] || fail "test release name"
assert_yq "$r4" '[select(.kind=="Workload")] | length' 0
# stage (Kargo branch) names the release when it differs from env, so dev-canary and dev never share an object
r5=$(render podinfo $c "${dev[@]}" --set stage=dev-canary --set mode=release --set image.tag=6.15.0)
...
# --- binding: one ReleaseBinding; the Environment (= cluster) is stamped later, placeholder until then
b=$(render podinfo $c "${dev[@]}" --set mode=binding --set image.tag=6.15.0)
assert_yq "$b" '[select(.kind != null)] | map(.kind) | join(",")' ReleaseBinding
assert_yq "$b" 'select(.kind=="ReleaseBinding") | .spec.releaseName' "$name"
assert_yq "$b" 'select(.kind=="ReleaseBinding") | .spec.environment' __CLUSTER__
assert_yq "$b" 'select(.kind=="ReleaseBinding") | .spec.owner.projectName + "/" + .spec.owner.componentName' lab/podinfo
assert_yq "$b" 'select(.kind=="ReleaseBinding") | .spec.componentTypeEnvironmentConfigs.replicas' 1
b=$(render podinfo $c "${dev[@]}" --set mode=binding --set image.tag=6.15.0 --set environment=dev1)
...
# a stamper that already knows the release name needs neither the image nor the frozen spec
b=$(render podinfo $c -f $a/app.yaml --set mode=binding --set environment=dev1 --set releaseName=podinfo-dev-1-2-3-abcdef12)
assert_yq "$b" 'select(.kind=="ReleaseBinding") | .spec.releaseName' podinfo-dev-1-2-3-abcdef12
...
assert_fails helm template x $c -f $a/app.yaml --set mode=release --set image.tag=1.0.0             # no env
assert_fails helm template x $c -f $a/app.yaml --set mode=release --set env=dev                     # no tag
assert_fails helm template x $c -f $a/app.yaml --set mode=release --set env=dev --set image.tag=1 --set image.repository=
assert_fails helm template x $c -f $a/app.yaml --set mode=release --set env=dev --set image.tag=1 --set container.image=x
assert_fails helm template x $c -f $a/app.yaml --set mode=binding --set env=dev                     # no tag, no releaseName
assert_fails helm template x $c -f $a/app.yaml --set mode=release --set env=dev --set image.tag=1 \
