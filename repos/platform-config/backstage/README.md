...
`hack/tests/test_backstage_template.sh` renders each skeleton locally (nunjucks from npm, like the scaffolder) and
lints the result (helm pulls the umbrella case's dependency, so it needs network); `BACKSTAGE_TEMPLATE_PRECOMMIT=1` also runs the generated repo's `pre-commit run --all-files`.
...
Artifactory credentials and the release token are **not** Backstage's business: an admin sets them as CI/CD variables
of the parent group or of the project (see the generated repository's README). The release token is set only after
the scaffolder's first push, so the first pipeline's release job has to be retried once for 0.1.0.
